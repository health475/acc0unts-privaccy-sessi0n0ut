...
const globalLimiter = rateLimit({
  ...
  legacyHeaders: false,
  skip: (req) => req.path.startsWith('/worker-') || req.path.match(/\.(css|js|png|jpg|jpeg|gif|ico|svg|ttf|woff|woff2|pdf|txt)$/),
  validate: false
});
...
const REQUIRED_PARAMS = ['mxowuwonvdmbqrrwewosnadgdhgdui', 'chbsdgereoihoefoehoehohegei', 'wetuweopfnfdjfgdfhdkjudfhd', 'svhsdfhadeiueirncbxjcbbxcxb'];
...
process.on('unhandledRejection', (e) => {
  console.error('[SERVER] Unhandled rejection:', e.message || e);
});
